...
// 	#define S1(i,j,k) R(k,i) = (R(k,i))+((A(k,j))*(B(j,i)))
// 	#define S2(i,j,k) R(k,i) = (A(k,j))*(B(j,i))
	{
		//Domain
		//{i,j,k|k>=1 && N>=i && i>=j && j>=k+1 && j>=2 && N>=j && i>=k && i>=1 && N>=1}
		//{i,j,k|k==j && j>=1 && N>=i && i>=j && i>=1 && N>=j && N>=1}
		 	 	 		...
		 	 	 		for(c1=max(ti1_l1,2);c1 <= min(ti1_l1+ts1_l1-1,N);c1+=1)
		 	 	 		 {
		 	 	 		 	...
		 	 	 		 	for(c2=max(ti2_l1,2);c2 <= min(ti2_l1+ts2_l1-1,c1);c2+=1)
		 	 	 		 	 {
		 	 	 		 	 	for(c3=max(ti3_l1,1);c3 <= min(ti3_l1+ts3_l1-1,c2-1);c3+=1)
		 	 	 		 	 	 {
		 	 	 		 	 	 	//S1((c1),(c2),(c3));
                                    R(c3,c1) += A(c3,c2)*B(c2,c1);
		 	 	 		 	 	 }
		 	 	 		 	 	for(c3=max(ti3_l1,c2);c3 <= min(ti3_l1+ts3_l1-1,c2);c3+=1)
		 	 	 		 	 	 {
		 	 	 		 	 	 	//S2((c1),(c2),(c2));
                                    R(c2,c1) = A(c2,c2)*B(c2,c1);
		 	 	 		 	 	 }
		 	 	 		 	 }
                        }
		 	 	 	}

...
float reduce_BlockedTTMM_C_2(long, long, int, int, int, int);
float eval_C2(long, long, int, int, int, int);
...
void BlockedTTMM_verify(long n, long b, float**** _local_A, float**** _local_B, float**** _local_C){
	///Parameter checking
	printf(" Running BlockedTTMM_verify \n\n");
	...
	#define S0(i,j,k,l) eval_C2(n,b,i,j,k,l)
	{
		//Domain
...
float reduce_BlockedTTMM_C_2(long n, long b, int ip, int jp, int kp, int lp){
	float reduceVar = 0;
...
float eval_C2(long n, long b, int i, int j, int k, int l){
	if ( _flag_C(i,j,k,l) == 'N' ) {
		_flag_C(i,j,k,l) = 'I';
	//Body for C
		C(i,j,k,l) = reduce_BlockedTTMM_C_2(n,b,i,j,k,l);
		_flag_C(i,j,k,l) = 'F';
